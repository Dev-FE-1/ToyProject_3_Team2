...
const BestPlaylists: React.FC<BestPlaylists> = ({ title, playlists }) => {
  ...
  const [playlistss, setPlaylists] = useState<PlaylistModel[]>([]);
  ...
  const filter = (playlists: PlaylistModel[]) =>
    [...playlists].sort((a, b) => b.likeCount - a.likeCount);

  console.log(filter(playlistss));
